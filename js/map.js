...
const SoundMap = {
  ...
  renderMarkers() {
    ...
    filtered.forEach(sound => {
      ...
      // Wrapper div prevents transform conflict with Mapbox's positioning
      const inner = document.createElement('div');
      inner.className = 'sound-marker-inner';
      inner.innerHTML = this._getMarkerEmoji(sound.tags);
      inner.style.cssText = `
        width: 36px;
        height: 36px;
        display: flex;
      ...
      el.addEventListener('mouseenter', () => {
        ...
        inner.style.boxShadow = '0 0 24px rgba(0, 212, 170, 0.6)';
      });
      el.addEventListener('mouseleave', () => {
        inner.style.transform = 'scale(1)';
        inner.style.boxShadow = '0 0 12px rgba(0, 212, 170, 0.3)';
      });
