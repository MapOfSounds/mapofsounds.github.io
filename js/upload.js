...
const Upload = {
  ...
  initUploadMap() {
    ...
    // Wait for page transition to finish (200ms) + a bit extra before initializing
    setTimeout(() => {
      ...
      // Force a resize so Mapbox renders correctly after page transition
      this.uploadMap.once('load', () => this.uploadMap.resize());
    }, 400);
  },
  ...
  _handleFile(file) {
    // Validate format — check MIME type OR file extension
    const validExts = /\.(mp3|wav|ogg|m4a|webm|flac|aac|opus|mp4|wma)$/i;
    if (!CONFIG.ALLOWED_FORMATS.includes(file.type) && !file.name.match(validExts)) {
      App.toast('Invalid audio format. Use MP3, WAV, OGG, M4A, FLAC, etc.', 'error');
      return;
    }
  ...
  async _submit() {
    ...
    if (!Auth.requireLogin('upload a sound')) return;
    ...
    try {
      ...
      // Use a longer timeout for uploads (60s)
