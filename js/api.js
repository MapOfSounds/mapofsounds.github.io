// ============================================
// MapOfSounds — API Client v3.0
// ============================================
// In DEMO_MODE (Worker unreachable), uses localStorage fallback.
// When Worker is live, hits the Cloudflare Worker backend with
// localStorage fallback on network failure.

const API = {
  baseUrl: CONFIG.API_URL,
  
  // Local storage fallback — always populated for offline resilience
  _localSounds: null,

  // Fetch timeout for API calls (ms)
  _FETCH_TIMEOUT: 8000,
  ...
  /** Fetch with timeout — never hang on a slow Worker. */
  ...
  async listSounds() {
    ...
    try {
      const res = await this._fetch(`${this.baseUrl}/list`);
      if (!res.ok) throw new Error('status ' + res.status);
      const data = await res.json();
      // Sync live data into localStorage for offline fallback
      if (Array.isArray(data) && data.length > 0) {
        this._localSounds = data;
        this._saveLocal();
      }
      return data;
    } catch (e) {
  ...
  async getSound(id) {
    ...
    try {
      const res = await this._fetch(`${this.baseUrl}/sound/${id}`);
      if (!res.ok) throw new Error('status ' + res.status);
  ...
  async searchSounds(query) {
    ...
    try {
      const res = await this._fetch(`${this.baseUrl}/search?q=${encodeURIComponent(query)}`);
      if (!res.ok) throw new Error('status ' + res.status);
      return await res.json();
    } catch (e) {
      return (this._localSounds || []).filter(s =>
  ...
  async uploadSound(formData) {
    ...
    // Use a much longer timeout for uploads (60 seconds)
    const controller = new AbortController();
    ...
    try {
      ...
      // Add to local cache immediately so it shows up right away
      if (result && result.id) {
        this._localSounds.push(result);
        this._saveLocal();
      }
      return result;
    } catch (e) {
  ...
  async likeSound(id) {
    ...
    if (!CONFIG.DEMO_MODE) {
      try { await this._fetch(`${this.baseUrl}/sound/${id}/like`, { method: 'POST' }); } catch (e) {}
    }
    return sound;
  },
  ...
  async addComment(id, author, text) {
    ...
    if (!CONFIG.DEMO_MODE) {
      try {
        await this._fetch(`${this.baseUrl}/sound/${id}/comment`, {
          method: 'POST',
