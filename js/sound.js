// ============================================
// MapOfSounds — Sound Detail Page v3.0
// ============================================
// Enhanced: download button, better layout
...
const SoundDetail = {
  ...
  async show(id) {
    ...
    const tagsHtml = sound.tags.map(t => {
      ...
      return `<span class="tag-chip clickable" onclick="App.navigate('search/tags'); setTimeout(() => Tags.selectTag('${t}'), 200)">${info ? info.emoji + ' ' : ''}${t}</span>`;
    }).join('');
    ...
    container.innerHTML = `
      <div class="sound-detail-hero">
        ...
        <div class="sound-detail-controls">
          ...
          <button class="btn btn-ghost" onclick="SoundDetail.download()">⬇️ Download</button>
        ...
        <div class="sound-detail-info">
          <div class="info-stat">
            <div class="info-stat-value">${(sound.plays || 0).toLocaleString()}</div>
  ...
  _initDetailWaveform(sound) {
    ...
    if (sound.audio) {
      ...
      this.detailWs.on('click', () => Player.play(sound));
    }
  },
  ...
  async addComment() {
    ...
    if (!Auth.requireLogin('post a comment')) return;

    const input = document.getElementById('comment-input');
    const text = input.value.trim();
    if (!text) return;
    
    const author = Auth.getUserName();
