...
const App = {
  ...
  async init() {
    try {
      ...
      // Navigate to initial page — skip auth callback hashes
      const hash = window.location.hash.slice(1) || 'map';
      if (!hash.startsWith('auth_token=') && !hash.startsWith('auth_error=')) {
  ...
  _initRouter() {
    window.addEventListener('hashchange', () => {
      ...
      // Ignore auth callback hashes (handled by Auth module)
      if (raw.startsWith('auth_token=') || raw.startsWith('auth_error=')) return;
      // '' or 'map' both mean home
      this.navigate(raw || '', false);
    });
  },
  
  navigate(route, pushHash = true) {
    const parts = route.split('/');
    let page = parts[0] || 'map';
    const param = parts[1] || null;

    // Normalize: '' and 'map' both map to the home (map) page
    if (page === '' || page === 'map') page = 'map';
    
    if (pushHash) {
      // For home page, use clean URL (no #map)
      if (page === 'map') {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        if (nearby.length === 0) {
          ...
          if (this.currentPage !== 'map') this.navigate('');
  ...
  _initKeyboard() {
    document.addEventListener('keydown', (e) => {
      ...
      // A = about
      if (e.key === 'a') this.navigate('about');
