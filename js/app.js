// ============================================
// MapOfSounds — Main App Controller v3.0
// ============================================
...
const App = {
  ...
  async init() {
    try {
      // 1. Fetch Mapbox token + config from Worker API (has 5s timeout)
      await CONFIG.fetchConfig();

      // 2. Initialize API / data
      API.init();

      // 3. Initialize modules — each wrapped so one failure doesn't block the rest
      ...
      console.log(
        CONFIG.DEMO_MODE
          ? 'MapOfSounds initialized (demo mode)'
          : 'MapOfSounds initialized (live API)'
      );
    } catch (err) {
      console.error('App init error:', err);
    } finally {
      // ALWAYS hide loading overlay — even if init partially failed
      setTimeout(() => {
  ...
  /** Safely initialize a module — logs error but never throws. */
  _safeInit(name, fn) {
    try {
      fn();
    } catch (err) {
